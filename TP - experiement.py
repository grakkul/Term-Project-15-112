...
def changeBackgroundColor(app):
    if app.score % 25 == 0 and app.score > 0:
        colorList = ['DarkSlateBlue','MediumPurple','DarkViolet','Purple','MediumVioletRed','HotPink','LightCoral','Tomato','Red','OrangeRed','Orange','Gold','Yellow','GreenYellow','LawnGreen','LimeGreen','MediumSpringGreen','Turquoise','LightSeaGreen']
        app.backColor = colorList[int(app.backColorNum)]
        app.backColorNum += (1/10)
            
        if app.backColorNum >= len(colorList):
            app.backColorNum = 0
...
def drawGrid(app,canvas):
    for row in range(app.rows):
        for col in range(app.cols):
            ...
            canvas.create_rectangle(x0,y0,x1,y1, outline = app.backOutline)
